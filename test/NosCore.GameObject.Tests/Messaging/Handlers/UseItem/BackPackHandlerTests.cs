...
{
    [TestClass]
    public class BackPackHandlerTests
    {
        private const short BackpackVNum = 1272;
        private ClientSession _session = null!;
        private BackPackHandler _handler = null!;
        ...
        {
            await new Spec("Item with an unrelated Effect is ignored (no StaticBonus grant, no item consumption)")
        ...
        [TestMethod]
        public async Task DuplicateBackPackGrantIsRejected()
        {
            await new Spec("Using a second BackPack item when one is already granted emits NotInPair and does not consume the item")
                .Given(ItemInInventoryWithEffect_, ItemEffectType.InventoryUpgrade)
                .And(CharacterAlreadyHasStaticBonus_, StaticBonusType.BackPack)
                .WhenAsync(UsingTheItem)
                .Then(NotInPairMessageShouldBeSent)
                .And(ItemStackCountShouldBe_, (short)1)
                .ExecuteAsync();
        }

        [TestMethod]
        public async Task DuplicateInventoryTicketGrantIsRejected()
        {
            await new Spec("Using a second InventoryTicketUpgrade item when one is already granted emits NotInPair")
                .Given(ItemInInventoryWithEffect_, ItemEffectType.InventoryTicketUpgrade)
                .And(CharacterAlreadyHasStaticBonus_, StaticBonusType.InventoryTicketUpgrade)
                .WhenAsync(UsingTheItem)
                .Then(NotInPairMessageShouldBeSent)
                .And(ItemStackCountShouldBe_, (short)1)
                .ExecuteAsync();
        ...
        [TestMethod]
        public async Task PermanentBackPackStoresNullDateEnd()
        {
            await new Spec("EffectValue=0 means permanent: StaticBonus.DateEnd stays null")
                .Given(ItemInInventoryWithEffect_, ItemEffectType.InventoryUpgrade)
        ...
        [TestMethod]
        public async Task TimedBackPackStoresDateEnd()
        {
        ...
        private void ItemInInventoryWithEffect_(ItemEffectType effect)
        {
            ...
            var inst = new ItemInstanceForTest(BackpackVNum) { Amount = 1, Item = item };
        ...
        private void ItemStackCountShouldBe_(short expected) =>
            Assert.AreEqual(expected, _session.Character.InventoryService.CountItem(BackpackVNum));

        private void NotInPairMessageShouldBeSent()
        {
