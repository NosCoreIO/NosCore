...
namespace NosCore.Tests.FriendAndBlacklistsTests
{
    ...
    public class BDelPacketHandlerTests
    {
        ...
        public void Setup()
        {
            ...
            _blackListController = new BlacklistController(_connectedAccountHttpClient.Object, _characterRelationDao, _characterDao.Object);
            _blackListHttpClient.Setup(s => s.GetBlackLists(It.IsAny<long>())).Returns((long id) => _blackListController.GetBlacklisted(id));
            _blackListHttpClient.Setup(s => s.DeleteFromBlacklist(It.IsAny<Guid>())).Callback((Guid id) => _blackListController.Delete(id));
        }
