...
{
    [UsedImplicitly]
    public sealed class BazaarMedalsHandler(IClock clock)
    {
        [UsedImplicitly]
        public async Task Handle(ItemUsedEvent evt)
        {
            var effect = evt.InventoryItem.ItemInstance.Item.Effect;
            if (effect != ItemEffectType.SilverNosMerchantUpgrade && effect != ItemEffectType.GoldNosMerchantUpgrade)
            {
                return;
            }
            ...
            session.Character.StaticBonusList.Add(new StaticBonusDto
            {
                ...
                StaticBonusType = effect == ItemEffectType.SilverNosMerchantUpgrade
                    ? StaticBonusType.BazaarMedalSilver
                    : StaticBonusType.BazaarMedalGold
            });
