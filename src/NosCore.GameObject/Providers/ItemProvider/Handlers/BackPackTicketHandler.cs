...
namespace NosCore.GameObject.Providers.ItemProvider.Handlers
{
    public class BackPackTicketHandler : IEventHandler<Item.Item, Tuple<InventoryItemInstance, UseItemPacket>>
    {
        ...
        {
            return (item.Effect == ItemEffectType.InventoryTicketUpgrade);
        }

        public void Execute(RequestData<Tuple<InventoryItemInstance, UseItemPacket>> requestData)
        {
            if (requestData.ClientSession.Character.StaticBonusList.All(s => s.StaticBonusType != StaticBonusType.InventoryTicketUpgrade))
            {
