...
namespace NosCore.Parser.Parsers
{
    public class I18NParser
    {
        ...
        {
            string regioncode = region.ToString().ToLower();
        ...
        public void InsertI18N(string folder)
        {
            ...
            {
                List<I18N_ActDescDTO> actdescdtos = new List<I18N_ActDescDTO>();
                try
                {
                    using (StreamReader stream = new StreamReader(I18NTextFileName(ActDescTxt, region), CodePagesEncodingProvider.Instance.GetEncoding(1252)))
                    {
                        while ((_line = stream.ReadLine()) != null)
                        {
                            string[] currentLine = _line.Split('\t');
                            if (actdesclist.FirstOrDefault(s => s.Key == currentLine[0] && s.RegionType == region) == null)
                            {
            ...
            foreach (RegionType region in Enum.GetValues(typeof(RegionType)))
            {
                List<I18N_CardDTO> carddtos = new List<I18N_CardDTO>();
                try
                {
                    using (StreamReader stream = new StreamReader(I18NTextFileName(CardTxt, region), CodePagesEncodingProvider.Instance.GetEncoding(1252)))
                    {
                        while ((_line = stream.ReadLine()) != null)
                        {
                            string[] currentLine = _line.Split('\t');
            ...
            foreach (RegionType region in Enum.GetValues(typeof(RegionType)))
            {
                List<I18N_BCardDTO> bcarddtos = new List<I18N_BCardDTO>();
                try
                {
                    using (StreamReader stream = new StreamReader(I18NTextFileName(BCardTxt, region), CodePagesEncodingProvider.Instance.GetEncoding(1252)))
                    {
                        while ((_line = stream.ReadLine()) != null)
                        {
                            string[] currentLine = _line.Split('\t');
                            if (bcardlist.FirstOrDefault(s => s.Key == currentLine[0] && s.RegionType == region) == null)
                            {
            ...
            foreach (RegionType region in Enum.GetValues(typeof(RegionType)))
            {
                List<I18N_ItemDTO> itemdtos = new List<I18N_ItemDTO>();
                try
                {
                    using (StreamReader stream = new StreamReader(I18NTextFileName(ItemTxt, region), CodePagesEncodingProvider.Instance.GetEncoding(1252)))
                    {
                        while ((_line = stream.ReadLine()) != null)
                        {
                            string[] currentLine = _line.Split('\t');
            ...
            foreach (RegionType region in Enum.GetValues(typeof(RegionType)))
            {
                List<I18N_MapIdDataDTO> mapiddatadtos = new List<I18N_MapIdDataDTO>();
                try
                {
                    using (StreamReader stream = new StreamReader(I18NTextFileName(MapIDDataTxt, region), CodePagesEncodingProvider.Instance.GetEncoding(1252)))
                    {
                        while ((_line = stream.ReadLine()) != null)
                        {
                            string[] currentLine = _line.Split('\t');
                            if (mapiddatalist.FirstOrDefault(s => s.Key == currentLine[0] && s.RegionType == region) == null)
                            {
            ...
            foreach (RegionType region in Enum.GetValues(typeof(RegionType)))
            {
                List<I18N_MapPointDataDTO> mappointdatadtos = new List<I18N_MapPointDataDTO>();
                try
                {
                    using (StreamReader stream = new StreamReader(I18NTextFileName(MapPointDataTxt, region), CodePagesEncodingProvider.Instance.GetEncoding(1252)))
                    {
                        while ((_line = stream.ReadLine()) != null)
                        {
                            string[] currentLine = _line.Split('\t');
                            if (mappointdatalist.FirstOrDefault(s => s.Key == currentLine[0] && s.RegionType == region) == null)
                            {
            ...
            foreach (RegionType region in Enum.GetValues(typeof(RegionType)))
            {
                List<I18N_NpcMonsterDTO> npcmonsterdto = new List<I18N_NpcMonsterDTO>();
                try
                {
                    using (StreamReader stream = new StreamReader(I18NTextFileName(MonsterTxt, region), CodePagesEncodingProvider.Instance.GetEncoding(1252)))
                    {
                        while ((_line = stream.ReadLine()) != null)
                        {
                            string[] currentLine = _line.Split('\t');
                            if (npcmonsterlist.FirstOrDefault(s => s.Key == currentLine[0] && s.RegionType == region) == null)
                            {
            ...
            foreach (RegionType region in Enum.GetValues(typeof(RegionType)))
            {
                List<I18N_NpcMonsterTalkDTO> npctalkdtos = new List<I18N_NpcMonsterTalkDTO>();
                try
                {
                    using (StreamReader stream = new StreamReader(I18NTextFileName(NpcTalkTxt, region), CodePagesEncodingProvider.Instance.GetEncoding(1252)))
                    {
                        while ((_line = stream.ReadLine()) != null)
                        {
                            string[] currentLine = _line.Split('\t');
                            if (npcmonstertalklist.FirstOrDefault(s => s.Key == currentLine[0] && s.RegionType == region) == null)
                            {
            ...
            foreach (RegionType region in Enum.GetValues(typeof(RegionType)))
            {
                List<I18N_QuestDTO> questdtos = new List<I18N_QuestDTO>();
                try
                {
                    using (StreamReader stream = new StreamReader(I18NTextFileName(QuestTxt, region), CodePagesEncodingProvider.Instance.GetEncoding(1252)))
                    {
                        while ((_line = stream.ReadLine()) != null)
                        {
                            string[] currentLine = _line.Split('\t');
                            if (questlist.FirstOrDefault(s => s.Key == currentLine[0] && s.RegionType == region) == null)
                            {
            ...
            foreach (RegionType region in Enum.GetValues(typeof(RegionType)))
            {
                List<I18N_SkillDTO> skilldtos = new List<I18N_SkillDTO>();
                try
                {
                    using (StreamReader stream = new StreamReader(I18NTextFileName(SkillTxt, region), CodePagesEncodingProvider.Instance.GetEncoding(1252)))
                    {
                        while ((_line = stream.ReadLine()) != null)
                        {
                            string[] currentLine = _line.Split('\t');
                            if (skilllist.FirstOrDefault(s => s.Key == currentLine[0] && s.RegionType == region) == null)
                            {
