                ...
                if (currentPacket[0].Equals("n_inv"))
                {
                    ...
                    for (int i = 5; i < currentPacket.Length; i++)
                    {
                        ...
                        if (item.Length == 5)
                        {
                            ...
                            {
                                ShopId = DaoFactory.GetGenericDao<ShopDto>().FirstOrDefault(s => s.MapNpcId == npcid).ShopId,
                                Type = type,
                        ...
                        if (sitem == null || shopitems.Any(s => s.ItemVNum.Equals(sitem.ItemVNum) && s.ShopId.Equals(sitem.ShopId)) 
                            || DaoFactory.GetGenericDao<ShopItemDto>().Where(s => s.ShopId == sitem.ShopId).Any(s => s.ItemVNum.Equals(sitem.ItemVNum)))
                        {
                            continue;
                        }
