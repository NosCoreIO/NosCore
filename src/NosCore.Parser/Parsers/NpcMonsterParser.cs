...
{
    class NpcMonsterParser
    {
        public void ImportNpcMonsters(string folder)
        {
			...
			string fileNpcId = $"{folder}\\monster.dat";
			string fileNpcLang = $"{folder}\\_code_uk_monster.txt"; //TODO: Multi Language
			...
			break;
			       ...
			       /* else if (currentLine.Length > 1 && currentLine[1] == "SKILL")
			        ...
			        {
			            if (DAOFactory.NpcMonsterDAO.FirstOrDefault(s => s.NpcMonsterVNum == npc.NpcMonsterVNum) == null)
			            {
			                npcs.Add(npc);
			                counter++;
			            }
			    ...
			    DAOFactory.NpcMonsterDAO.InsertOrUpdate(npcs);
                // DAOFactory.NpcMonsterSkillDAO.Insert(skills);
                // DAOFactory.BCardDao.Insert(monstercards);
                Logger.Log.Info(string.Format(LogLanguage.Instance.GetMessageFromKey(LanguageKey.NPCMONSTER_PARSED), counter));
                npcIdStream.Close();
			}
